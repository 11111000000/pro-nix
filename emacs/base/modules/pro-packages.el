...
" 
  (interactive)
  (let* ((defaults '(consult corfu cape treemacs projectile expand-region yasnippet consult-yasnippet consult-eglot kind-icon vterm exwm eldoc-box))
         (wanted (or packages defaults))
         (provided (when (boundp 'pro-packages-provided-by-nix) pro-packages-provided-by-nix)))
    (dolist (p wanted)
      (unless (memq p provided)
        (unless (pro--package-provided-p p)
          (message "pro-packages: ensuring %s" p)
