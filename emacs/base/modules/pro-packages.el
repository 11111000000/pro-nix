...
;; Назначение: минимальный движок для разрешения пакетов с приоритетом runtime/manual.
;;
;; Контракт:
;; - pro-packages--maybe-install / pro/packages-ensure — публичные точки входа для проверки/установки пакетов.
;; - Политика: runtime/manual > Nix-provided > MELPA/ELPA по необходимости > package-vc (last resort).
...
(defconst pro-packages-archives
  '(("gnu" . "https://elpa.gnu.org/packages/")
    ("nongnu" . "https://elpa.nongnu.org/nongnu/")
    ("melpa" . "https://melpa.org/packages/"))
  "Архивы пакетов, доступные при старте Emacs.")
...
(defvar pro-packages--initialized nil
  "Non-nil once `package-initialize' has been called in this session.")

(defvar pro-packages-refresh-ttl 86400
  "Порог устаревания кэша архивов в секундах. NIL отключает TTL-проверку.")

(defvar pro-packages-refresh-stamp-file
...
(defun pro-packages--refresh-needed-p ()
  "Вернуть non-nil, если refresh архивов нужен перед установкой."
  (or (not (bound-and-true-p package-archive-contents))
      (not package-archive-contents)
      (not (file-exists-p pro-packages-refresh-stamp-file))
      (and pro-packages-refresh-ttl
           (file-exists-p pro-packages-refresh-stamp-file)
           (> (- (float-time)
                 (float-time (file-attribute-modification-time
                              (file-attributes pro-packages-refresh-stamp-file)))))
              pro-packages-refresh-ttl)))

(defun pro-packages-refresh-if-needed (&optional force)
  "Обновить списки архивов только при реальной необходимости.
FORCE принудительно запускает refresh."
  (when (or force (pro-packages--refresh-needed-p))
    (condition-case err
        (progn
          (package-refresh-contents)
          (setq pro-packages--refreshed t)
          (make-directory (file-name-directory pro-packages-refresh-stamp-file) t)
          (with-temp-file pro-packages-refresh-stamp-file
            (insert (format-time-string "%FT%T%z")))
          t)
        (error (message "[pro-packages] failed to refresh package archives: %S" err) nil))))

(defun pro-packages--archives-configured-p ()
  "Вернуть non-nil, если нужные архивы уже присутствуют в `package-archives'."
...
(defun pro--package-runtime-available-p (pkg)
  "Return non-nil if PKG is already available from runtime/manual install."
  (or (package-installed-p pkg)
      (locate-library (symbol-name pkg))
      (condition-case nil
          (require pkg nil t)
...
(defun pro/packages-ensure (pkg &optional allow-melpa)
  "Ensure PKG is available following the policy: runtime/manual -> Nix -> MELPA -> Git.
...
(defun pro-packages-refresh ()
  ...
  (interactive)
  (condition-case err
      (progn (pro-packages-refresh-if-needed t) (message "pro-packages: refreshed package archives") t)
...
" 
  (interactive)
    ...
    (dolist (p wanted)
      ...
      (unless (pro--package-runtime-available-p p)
