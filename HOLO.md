...
22. INV-Emacs-Package-Availability
 ...
 23. INV-Opencode-Isolated
    Opencode запускается только через `programs.opencode-bwrap` как sandboxed Home Manager wrapper; декларативный пакет и опции приходят из upstream `opencode-bwrap-nix`, а проверка происходит через `./scripts/opencode-smoke.sh` и `tests/contract/unit/04-opencode-options.sh`.
