...
# Модуль конфигурации opencode для хостов.
# Учебный комментарий: модуль предоставляет системный бинарник/обёртку opencode
# и опции для интеграции агентов. Инвариант: модули лишь добавляют вклад
# через lib.mkDefault; окончательная фиксация списка пакетов должна происходить
# на уровне host (configuration.nix) через lib.mkForce при необходимости.
