...
- Вложенные .git в analyse/ и angular.js: репозиторий содержит директории с собственными git-объектами (packs), что сильно увеличивает размер и усложняет управление. Эти каталоги выглядят как импортированные копии upstream-проектов (vendor) и должны быть вынесены.
