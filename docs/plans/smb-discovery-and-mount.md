...
- Не храните креды в Git; пользуйтесь /etc/pro-peer и GPG flow (pro-peer already used for authorized_keys). Можно расширить эту модель для зашифрованных /etc/samba/creds.d/<host>.gpg и автоматической расшифровки при установке (аналогично ops-pro-peer-sync-keys.sh).
...
- Принять этот документ как план в docs/plans/ и если согласны — я добавлю:
  ...
  2) поддержку зашифрованных creds (/etc/samba/creds.d/*.gpg) с аналогичным ops-pro-peer-sync-keys.sh flow для развёртывания секретов.
...
- Не коммитим креды и ключи. (Добавил правило в AGENTS.md). Скрипты, которые управляют creds, должны работать с зашифрованными артефактами и operator workflows (ops-pro-peer-sync-keys.sh — шаблон).
- В репо должны быть только шаблоны unit/инструкции и скрипты мастера, которые принимают подготовленные, зашифрованные файлы и безопасно их раскладывают (аналогично pro-peer-master.sh и ops-pro-peer-sync-keys.sh).
