...
- Если нежелательно хранить пароли в явном виде на диске, рассмотреть зашифрованный файл /etc/samba/creds.d/<host>.gpg и автоматическую расшифровку (см. pro-peer-sync-keys.sh как пример). Это потребует расширения модульного flow и операторской работы.
