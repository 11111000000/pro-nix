...
Глобальный план (помимо ключей): терминалы, вкладки, окна, Nix-first
=====================================================================
Цель: сохранить минимализм и надёжность, но не потерять рабочие вещи из ~/pro,
которые заметно улучшают повседневную жизнь. Главный принцип: сначала Nix,
потом Emacs-конфиг; сначала поведение, потом косметика.

Диалектический критерий отбора
- Переносим не то, что "интересно", а то, что:
  1. используется каждый день,
  2. даёт крупный выигрыш при малом коде,
  3. не ломает воспроизводимость,
  4. можно включать/выключать как модуль.

Приоритет 1 — Nix-first ядро
- Все ключевые пакеты должны приходить из Nix, а не через случайный ELPA-install:
  - completion/navigation: consult, consult-dash, consult-eglot, consult-yasnippet,
    vertico, orderless, marginalia, corfu, cape, kind-icon, avy, expand-region,
    projectile, treemacs, yasnippet.
  - terminal/window helpers: vterm, ace-window.
  - system tools: ripgrep, fd, findutils.
- Пакеты уже частично добавлены; финальная задача — собрать короткий список
  «must-have via Nix» и проверить, что home-manager реально их устанавливает.

Приоритет 2 — Терминалы
- Из ~/pro реально ценное:
  - vterm как основной терминал,
  - корректная вставка/yank в vterm,
  - interrupt helper,
  - разумное поведение tab-line внутри терминала,
  - multi-vterm — опционально, только если пакет есть в Nix.
- Что НЕ переносим сейчас:
  - длинные фиксы kill-ring/copy-mode, пока нет подтверждённой боли.
  - сложную кастомную тему vterm, пока нет потребности.
- Что делаем:
  - terminals.el уже добавлен как минимальный модуль.
  - Следующий шаг: если vterm доступен из Nix, добавить один-два локальных helper'а
    для copy-mode и документировать рекомендуемые клавиши в emacs-keys.org.

Приоритет 3 — Вкладки
- Из ~/pro ценно не само pro-tabs как внешний проект, а поведение:
  - tab-bar включён,
  - удобное открытие/закрытие/переключение вкладок,
  - optional icons,
  - tab-line для буферов в окне.
- Что делаем:
  - pro-tabs.el уже сделан как thin wrapper.
  - Если внешний `pro-tabs` недоступен через Nix, используем built-in tab-bar/tab-line.
  - Иконки только если графика и пакет доступен.
- Принцип: вкладки — opt-in, а не обязательный слой.

Приоритет 4 — Окна и попапы
- Из ~/pro реально полезно:
  - winner-mode,
  - ace-window,
  - golden-ratio (спорно, поэтому opt-in),
  - предсказуемое размещение терминалов/служебных буферов.
- Что НЕ нужно сейчас:
  - большой popper/popwin пласт без ясной необходимости.
- Что делаем:
  - windows.el уже добавлен.
  - Следующий минимальный шаг: добавить только небольшой `display-buffer-alist`
    для vterm и, возможно, *Messages* — без общей popper-экосистемы.

Приоритет 5 — EXWM
- Из ~/pro важно:
  - simulation keys,
  - rename hooks,
  - базовая интеграция системного лотка/окон.
- Но EXWM — большой пласт, поэтому переносим по остаточному принципу:
  - сначала только simulation keys и безопасные hooks,
  - без разрастания в отдельную полу-ОС.

Приоритет 6 — Косметика
- Косметика допустима только после ядра:
  - kind-icon/corfu margin,
  - tab icons,
  - vterm theme,
  - golden-ratio defaults.
- Если что-то ломает предсказуемость, это не косметика, а шум — не включать.

Итоговый минимальный roadmap
1. Завершить Nix-first список и проверить, что всё тянется из Nix.
2. Довести terminals.el до минимально удобного состояния.
3. Довести pro-tabs.el/widows.el до состояния "простые wrappers + docs".
4. Добавить очень малый display-buffer policy для терминалов.
5. Потом вернуться к EXWM simulation keys.

Критерий красоты
- Конфиг прекрасен, когда:
  - его можно объяснить за 5 минут,
  - в нём нет скрытой магии,
  - всё важное ставится из Nix,
  - глобальные клавиши лежат в одном Org-файле,
  - новые модули можно отключить одной переменной.
