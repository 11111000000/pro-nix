...
Принцип отбора
- Пакет считается частью базовой системы только если он:
  - решает ежедневную задачу,
  - доступен через Nix,
  - не требует сложной пользовательской инфраструктуры,
  - может отсутствовать без краха всей конфигурации.
...
- тяжёлых popup-фреймворков по умолчанию (`popper`, `popwin`)
- внешних пакетов, живущих только в одном домашнем каталоге
- декоративных пакетов без явной ежедневной пользы
