...
testers.nixosTest {
  ...
  nodes.machine = { config, pkgs, lib, ... }: {
    ...
    # Отключаем tor, чтобы избежать зависимостей
  ...
  testScript = ''
    ...
    if "Unbalanced quoting" in out:
        raise Exception(f"Unbalanced quoting found: {out}")
    
    # Проверка ExecStart
    exec_out = machine.succeed("grep ExecStart /etc/systemd/system/tor-ensure-bridges.service")
    if "/bin/sh -c" in exec_out:
        raise Exception(f"Wrong ExecStart: {exec_out}")
    if "/nix/store" not in exec_out:
        raise Exception(f"ExecStart missing /nix/store: {exec_out}")
