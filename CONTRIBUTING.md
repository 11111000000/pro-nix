...
Рекомендация: использовать отдельные ветки и, при необходимости, git worktree для разделения рабочих каталогов. Требование о строгом использовании linked worktree снято; допустима работа в primary worktree при явном согласии оператора. Конвенция "one agent = one branch" остаётся рекомендуемой практикой для ясности, но не является обязательной.
