...
- templates/ — шаблоны пользовательских конфигураций, распространяемые через pro-nix
...
7. Шаблоны пользовательских конфигов
   - Пользовательские шаблоны живут в `templates/` и распространяются через
     NixOS‑политику на все хосты.
   - При `switch` шаблон попадает в `/etc/skel/pro-templates`, а Home Manager
     копирует его в домашний каталог только если файла ещё нет.
