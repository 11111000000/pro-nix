...
0. Перед запуском `team-run` агент обязан привести рабочее дерево в чистое состояние (commit или stash).

1. Прочитать `AGENTS.md`, `SURFACE.md`.
...
- Документация без изменения поведения: `nix fmt --check` или визуальный осмотр.
- `SURFACE.md` или Proof-реестр: только затронутый Proof.
- `HOLO.md` или процессные правила: визуальный осмотр.
...
Для мелких правок (Pressure=Debt, Surface impact: none) достаточно Intent + Proof.
...
Минимальный diff на один Intent. Предпочитать существующий паттерн проекта. Большие изменения делить на этапы: Surface → Proof → Code → Verify.
...
Работа в primary worktree допустима; рекомендуется отдельная ветка.
Для критичных изменений обязателен Change Gate и Proof.
