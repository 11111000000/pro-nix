...
- Цель: исключить совместную работу нескольких агентов в одном рабочем дереве и сделать контекст каждой правки изолированным.
- Правило: любой агент, который меняет репозиторий, обязан работать в linked worktree, а не в primary worktree.
- Инвариант: primary worktree используется как операционная база репозитория; рабочие изменения, коммиты и PR выполняются из отдельных linked worktree.

Порядок работы:

1. В корне репозитория выполнить `./scripts/check-worktree.sh`.
2. Если скрипт вернул код `2`, создать linked worktree командой `./scripts/setup-worktree.sh <ветка> [<каталог>]`.
3. Перейти в созданный каталог и выполнять все изменения, проверки, коммиты и push только там.

Proof и автоматизация:

- `./scripts/check-worktree.sh --help`
- `./scripts/setup-worktree.sh --help`
- рекомендуется вызывать `./scripts/check-worktree.sh` в preflight, pre-commit и pre-push для агентных сценариев.
