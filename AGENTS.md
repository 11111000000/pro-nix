...
Правило: агент должен предпочитать изолированную ветку и рабочий каталог, но
разрешено выполнять изменения в primary worktree при явном согласии оператора.

Рекомендации для агентов:

- Старайтесь работать в отдельной ветке и аккуратно формируйте коммиты.
- Перед массовыми или рискованными изменениями создавайте временную ветку
  и ставьте пометку в PR/коммите о том, что изменения выполнены в primary worktree.
- Для автоматизированных run'ов и CI продолжать соблюдать минимальные Verify и Change Gate.

Остальные детали работы с git worktree и автоматизацией оставлены в CONTRIBUTING.md
как опциональная практика, но не обязательный guard.
