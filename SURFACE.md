...
- Имя: Emacs Package Resolution
  Стабильность: [FLUID]
  Спецификация: при старте Emacs сначала подключаются `gnu`, `nongnu` и `melpa`, затем выполняется `package-initialize`, чтобы вручную установленные пакеты получили приоритет в `load-path`; пакеты из Nix используются как декларативный fallback, а `package-refresh-contents` выполняется только по необходимости, когда требуется установка отсутствующего пакета или когда пользователь явно запрашивает обновление архивов.
  Proof: `./scripts/test-run-emacs-e2e.sh`
  Owner: `emacs/base/init.el`, `emacs/base/modules/pro-packages.el`, `emacs/base/modules/pro-package-bootstrap.el`, `emacs/base/tests`
