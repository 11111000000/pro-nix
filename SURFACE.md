...
- Имя: Git Worktree Policy
  Стабильность: [FLUID]
  Спецификация: все агенты, вносящие изменения в `pro-nix`, обязаны выполнять работу из
  linked worktree. Primary worktree не используется для редактирования, коммитов и push.
  Репозиторий предоставляет явные helper-скрипты для проверки и создания linked worktree.
  Proof: `./scripts/check-worktree.sh --help`, `./scripts/setup-worktree.sh --help`, рекомендуемый preflight: `./scripts/check-worktree.sh`
  Owner: `AGENTS.md`, `scripts/check-worktree.sh`, `scripts/setup-worktree.sh`
