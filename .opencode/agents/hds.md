...
Behaviors:
 - When invoked, load project instructions from `opencode.json` (instructions array) if present and respect the seed located there (usually `docs/hds-llm-seed-en.md`). If the directory `/home/az/Code/HDS` exists on the developer host, prefer its seed and configuration (for example `/home/az/Code/HDS/opencode.json` or `/home/az/Code/HDS/docs/hds-llm-seed-en.md`) and load that before local project seeds. This allows using a central HDS source during development.
